...
优先用 `--hotel-id`，并带上日期与入住人数，才能更接近真实房型与取消规则。
...
当用户说"设计感""亲子友好""早餐好""品牌控"这类模糊偏好时，先查标签再搜索。
...
- `--place-type` 必须使用 `search-hotels --help` 里给出的精确值
- `hotel-detail` 不支持 `--format table`
- `--check-out-date` 必须晚于 `--check-in-date`
- 有 `hotelId` 时优先用 `hotelId`
- 不要只靠 `--name` 的模糊匹配直接比价，先确认具体酒店实体
- 没有真实历史波动数据时，不要编百分比降幅
- 如果 agent 有 `Heartbeat` 或 `Cron`，优先用它们承接定时盯价，不要只停在一次性摘要

## 排查

- **提示缺少 API Key：** 设置 `RollingGo_API_KEY`、显式传 `--api-key`，或者如果你跑在任何 claw 风格宿主里，就按 [claw-host-env.md](claw-host-env.md) 把同一个 key 注入到那个宿主的配置层
- **搜不到结果：** 先放宽星级、标签、距离、预算，再考虑放宽日期
- **详情里没有可售房型：** 这是业务结果，不是报错；换日期、人数或酒店重试
